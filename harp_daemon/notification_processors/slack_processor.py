...
class GenerateAutoSlack(object):
	def __init__(self, event_id, rendered_graph, notification_output, object_name, service, alert_name, alert_id, slack_id, description, studio, graph_url=None, additional_fields=None):
	...
	def render_image(self):
		if self.rendered_graph:
			...
			file_img = settings.SLACK_TEMP_FILES + "/{0}.png".format(time_now)
	...
	def close_chat_comment(self):
		...
		log.debug(
			msg=f"Close Slack Chat",
			extra={"tags": {}}
		)

		self.slack_processor(action='close')


class VerifySlack(object):
	def __init__(self, slack_id, action_type, alert_id):
		self.slack_id = slack_id
		self.bot = None
		...
		if self.action_type == 'assign':
			try:
				for slack_chat in self.slack_id:
					event_body = {
						'slack_channel': slack_chat,
						'title': f'Alert has been assigned to Slack chat - {slack_chat}',
					...
					log.info(
						msg=f"Slack chat has been verified: {slack_chat}\nResult: {req.json()}"
					)
