...
from harp_daemon.notification_processors.voice_processor import GenerateAutoVoice
...
class VoiceHandler(object):
    ...
    def create_event(self):
        ...
        NotificationHistory.update_alert_history(
            alert_id=self.notification['exist_alert_body']['id'],
            notification_output=f"Phone: {', '.join(self.notification['procedure']['ids'])}",
            notification_action="Created PhoneCall event"
        )
    ...
    def update_event(self):
        ...
        NotificationHistory.update_alert_history(
            alert_id=self.notification['exist_alert_body']['id'],
            notification_output=f"Phone: {', '.join(self.notification['procedure']['ids'])}",
            notification_action="Updated PhoneCall event"
        )
    ...
    def resubmit_event(self):
        ...
        NotificationHistory.update_alert_history(
            alert_id=self.notification['exist_alert_body']['id'],
            notification_output=f"Phone: {', '.join(self.notification['procedure']['ids'])}",
            notification_action="Resubmitted PhoneCall event"
        )
