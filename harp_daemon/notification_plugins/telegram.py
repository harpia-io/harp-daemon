...
from harp_daemon.notification_processors.telegram_processor import GenerateAutoTelegram
...
class TelegramHandler(object):
    ...
    def process_telegram(self, action, telegram_id=None, description=None):
        ...
        process_event = GenerateAutoTelegram(
    ...
    def create_event(self):
        ...
        self.process_telegram(action='create_event')

        NotificationHistory.update_alert_history(
            alert_id=self.notification['exist_alert_body']['id'],
            notification_output=f"Telegram ID: {', '.join(self.notification['procedure']['ids'])}",
            notification_action="Created Telegram event"
        )
    ...
    def resubmit_event(self):
        ...
        self.process_telegram(action='still_exist')
        ActiveAlerts.update_exist_event(data=self.update_alerts_template(), event_id=self.notification['exist_alert_body']['id'])

        NotificationHistory.update_alert_history(
            alert_id=self.notification['exist_alert_body']['id'],
            notification_output=f"Telegram ID: {', '.join(self.notification['procedure']['ids'])}",
            notification_action="Resubmitted Telegram event"
        )
    ...
    def close_event(self):
        ...
        log.info(
            msg=f"Receive event to close Telegram event\n{self.notification}",
            extra={"tags": {}}
        )

        try:
            self.process_telegram(action='close_event')
        except Exception as err:
            log.error(f"Cannot close Telegram event - {err}")
